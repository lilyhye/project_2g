...
adjusted_score = (crisis_sharpe - crisis_sharpe.min()) + 0.1

# 금(Gold)과 달러(Dollar) 같은 전통적 안전자산 프리미엄 부여 (알고리즘적 조정)
safe_assets = ['Gold', 'Dollar']
for asset in safe_assets:
    if asset in adjusted_score:
        adjusted_score[asset] *= 1.3 # 방어 목적이므로 안전자산에 30%의 추가 알고리즘 비중 부과

# 최종 비중 산출 (100% 합계 기준)
optimal_weights = (adjusted_score / adjusted_score.sum()) * 100
# ==============================================================
...
if not recent_returns.empty:
    ...
    # 2) 추천 포트폴리오 수익률 계산 (일일수익률 * 추천비중 합산)
    ...
    recent_cum['🌟 방어형 추천 포트폴리오'] = port_cum_returns
    
    # 3) 선 그래프 그리기
    fig5 = px.line(recent_cum * 100, x=recent_cum.index, y=recent_cum.columns,
                 labels={'value':'누적 수익률 (%)', 'Date':'날짜', 'variable':'투자 대상'},
                 title='개별 자산 vs 단순 25% 분배 vs 방어형 추천 포트폴리오 방어력 비교')
    # 포트폴리오 선 강조 (방어형은 두꺼운 실선, 균등형은 두꺼운 점선)
    fig5.update_traces(line=dict(width=4), selector=dict(name='🌟 방어형 추천 포트폴리오'))
    fig5.update_traces(line=dict(width=3, dash='dot'), selector=dict(name='⚖️ 균등 분배 포트폴리오(25%씩)'))
    st.plotly_chart(fig5, use_container_width=True)
    
    # 4) 4/3일 기준 결과 메트릭 카드 표시
    st.markdown("#### 📌 4월 3일 기준 최종 누적 수익률")
    cols = st.columns(len(recent_cum.columns))
    for i, col_name in enumerate(recent_cum.columns):
        final_return = recent_cum.iloc[-1][col_name] * 100
        cols[i].metric(label=col_name, value=f"{final_return:.2f}%")
        
    st.info("ℹ️ **[데이터 가공 정보]** 2026년 2월 27일부터 4월 3일까지의 실제 시장 데이터 상에서, 도출된 최적 가중치를 반영한 '방어형 추천 포트'와 무작위로 25%씩 배분한 '단순 균등 포트(1/N)'의 누적 수익률을 동시 산출하여 시각적으로 비교 연산했습니다.\n\n"
               "💡 **[그래프 해석 가이드]** 굵은 실선과 굵은 점선 두 개를 집중해서 비교해 보세요. 그냥 25%씩 아무 생각 없이 나눠 담은 것(점선) 보다, 위기 방어력을 최우선으로 고려한 방어형 포트폴리오(실선)가 흔들림을 막아주거나 수익을 얼마나 더 끌어올렸는지를 한눈에 체감하실 수 있을 겁니다.")
else:
